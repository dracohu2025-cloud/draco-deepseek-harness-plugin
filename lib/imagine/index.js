...
* under `$DSH_HOME/draco/videos/` (the attachment store is image-only).
...
function apply(ctx, config) {
	...
	ctx.tools.register(defineTool({
		name: "video_generate",
		description: "Generate a short video from a text prompt. SuperGrok login defaults to grok-imagine-video-1.5. Settings → Draco-suite chooses the backend. The MP4 is written under $DSH_HOME/draco/videos/.",
		parameters: {
			prompt: {
				type: "string",
				required: true,
		...
		async execute(args, exec) {
			...
			const path = dshHomePath(join("draco", "videos", `grok-imagine-video-1.5-${Date.now()}.mp4`));
			mkdirSync(dirname(path), { recursive: true });
