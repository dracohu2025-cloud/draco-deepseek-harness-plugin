...
* `video_generate`. Image and video backends live in the `draco-image-gen`
* settings namespace (`provider` vs `videoProvider`). Completing a Codex
* OAuth login defaults an unset image backend to Codex `gpt-image-2`;
* completing a SuperGrok login defaults an unset image backend to Grok
* Imagine Image 2.0 and an unset video backend to Imagine Video 1.5. An explicit
* user choice is left alone. Generated rasters are committed through
* `ctx.attachments` before the tool result is appended. Videos are written
* under `$DSH_HOME/draco/videos/`.
...
/** Shared registry Seedance (and later video plugins) advertise on. */
...
function apply(ctx, config) {
	...
	ctx.tools.register(defineTool({
		name: "video_generate",
		timeoutMs: 6e5,
		description: "Generate a short video. Settings → Draco-suite chooses grok-imagine-video-1.5 or doubao-seedance-2.0 / mini / fast. Optional references (up to 7): Imagine uses them as style stills; Seedance uses the first as the first frame. Pass image_generate paths, sha256 attachment ids, https URLs, data URIs, or \"latest\". The MP4 is written under $DSH_HOME/draco/videos/.",
		parameters: {
			prompt: {
				type: "string",
				required: true,
		...
		async execute(args, exec) {
			const prompt = args.prompt.trim();
			if (prompt.length === 0) throw new Error("prompt must be a non-empty string");
			if (current.videoProvider !== "xai-imagine") {
				const run = ctx.tools[VIDEO_BACKENDS]?.get(current.videoProvider);
				if (run !== void 0) return run(args, exec);
				throw new Error("video generation is not configured; pick a backend in Settings → Draco-suite");
			}
			const bearer = await resolveXaiBearer(ctx);
			if (bearer === void 0) throw new Error("no SuperGrok OAuth session or XAI_API_KEY; run /grok-login or use Settings → Draco-suite → Sign in with SuperGrok");
