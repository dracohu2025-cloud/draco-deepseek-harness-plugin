...
function apply(ctx) {
	...
	const announceReady = (expiresAt) => {
		ctx.emit("draco/xai-oauth-ready", expiresAt);
	};
	const loadTokens = () => {
		...
		if (tokens !== void 0) {
			...
			if (Date.now() + REFRESH_SKEW_MS < tokens.expiresAt) announceReady(tokens.expiresAt);
		}
		return tokens;
	};
	...
	const service = {
		...
		startLogin: async () => {
			...
			(async () => {
				try {
					const next = await pollForToken(ticket, await discoverTokenEndpoint(), controller.signal);
					tokens = next;
					writeTokensToDisk(next);
					status = {
						kind: "active",
