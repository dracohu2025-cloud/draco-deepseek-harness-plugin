window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function speechProbeOf(value) {
			if (value === "checking" || value === "ok" || value === "fail") return value;
			return "idle";
		}
		/**
		* Settings field that starts a probe for the selected backend.
		* @param value - selected dropdown row.
		* @returns the probe field, or `undefined` when speech is off.
		*/
		function speechProbeFieldOf(value) {
			if (value === "doubao-tts") return "doubaoProbe";
			if (value === "seed-audio") return "seedProbe";
		}
		/**
		...
		*/
		function SpeechGenPickerCard({ t, setValue, saveKeys, useStore }) {
			...
			const doubaoProbe = useStore((s) => s.doubaoProbe);
			const seedProbe = useStore((s) => s.seedProbe);
			const doubaoProbeError = useStore((s) => s.doubaoProbeError);
			const seedProbeError = useStore((s) => s.seedProbeError);
		...
		*/
		function createSpeechGenPickerStore() {
			return (0, _deepseek_ai_dsh_client_runtime_client.defineStore)({
				...
				actions: { sync: (draft, next) => {
					...
					draft.doubaoProbe = next.doubaoProbe;
					draft.seedProbe = next.seedProbe;
					draft.doubaoProbeError = next.doubaoProbeError;
					draft.seedProbeError = next.seedProbeError;
				} }
			});
		}
		//#endregion
		...
		function snapshotOf(scope, seat) {
			...
			return {
				...
				doubaoProbe: speechProbeOf(snap.value?.doubaoProbe),
				seedProbe: speechProbeOf(snap.value?.seedProbe),
				doubaoProbeError: snap.value?.doubaoProbeError ?? "",
				seedProbeError: snap.value?.seedProbeError ?? ""
			};
		}
		/**
		...
		*/
		function installSpeechGenPicker(ctx) {
			...
			const kickIdleProbe = () => {
				...
				const field = speechProbeFieldOf(snap.value);
				if (field === void 0) return;
				const configured = field === "doubaoProbe" ? seat.doubaoConfigured : seat.seedConfigured;
				const status = field === "doubaoProbe" ? snap.doubaoProbe : snap.seedProbe;
				if (configured && status === "idle") scope.set(field, "checking");
			};
			...
			const saveKeys = credentials === void 0 ? void 0 : async (keys) => {
				...
				await refreshKeys();
				const field = speechProbeFieldOf(speechValueOf(scope.getSnapshot().value));
				if (field === void 0) return;
				const errorField = field === "doubaoProbe" ? "doubaoProbeError" : "seedProbeError";
				await scope.set(errorField, "");
				await scope.set(field, "checking");
			};
